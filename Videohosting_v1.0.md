...
Диаграмма компонентов включает архитектурные блоки:
1. Клиентский уровень
* Web UI (поиск, плеер, регистрация, ЛК)
* Video Player (логический компонент)
2. Backend (бизнес-логика)
* API Gateway
1. Сервисы
* Auth Service (регистрация, вход, RBAC)
* User Service
* Video Management Service (CRUD видео)
* Search Service
* Streaming Service
* Like Service
* Comment Service
* Statistics Service
1. Обработка видео
* Upload Service (resumable upload, 50 ГБ)
* Transcoding Service (битрейты)
* Message Queue (асинхронность)
1. Инфраструктура
* User DB
* Video Metadata DB
* Object Storage
